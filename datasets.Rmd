...
d91_clean <- d91 %>% 
  rename(geo = Géographie, # just choosing more basic variable names
         sex = Sexe..3.) %>% 
  filter(str_detect(geo, "[:alpha:]"), # we are keeping observations with names, because those without names are census subdivisions
         str_detect(sex, "Total"), # we only keep the total values for sex -- we do not need to know the values for men and women separately
         !str_detect(geo, "Newfoundland | Terre-Neuve"), # removing observations that are the provinces 
         ...
         !str_detect(geo, "Canada \\(00\\)")) %>% # removing the Canada observation 
  gather(age, value, Total...Groupes.d.âge:X90.et.plus) %>% # transforming from wide to long
  filter(!str_detect(age, "Total")) %>% # removing observations that are "age group totals"
  mutate(age = str_remove(age, "X")) %>% # removing the "X" in front for each age category
  dplyr::select(geo, age, value) # keeping the 3 variables of interest -- we now have a value (obs.) for each electoral district and age category

table(d91_clean$age) # just looking through the age variable to check what's remianing

d91_clean <- d91_clean %>% # people aged less than 1 year are identified as "less than one year" and people aged more than 90 are categorized as "more than 90"
  mutate(age = ifelse(age == "Moins.de.1", 0, age), # recoding these two, so that "< 1 year" = "0" and "> 90" = "90"
         age = ifelse(age == "90.et.plus", 90, age)) %>% 
  filter(!str_detect(age, "\\.")) # age categories with periods in them correspond to age groups -- we don't want them, we only want single ages (except for the 90+ age group)

table(d91_clean$age) # seems pretty good
class(d91_clean$age) # I have a feeling this variable is not numeric, because I just cleaned up words in it
...
d91_clean <- d91_clean %>% 
  mutate(census = 1991, # adding a variable telling us which census it is
         ro = 1987) # adding a variable telling is which representation it is (I removed it later, turns out I did not need it)

# Here's an important step. The "geo" variable has the names of district _and_ their numbers (in parentheses). 
# I want to keep these two pieces of information, but in two different variables, because I may need one of the other
# for merging purposes later on. I will therefore split this variable into two (dist_name and dist_nb), then
# clean them up (especially district names).
d91_clean <- d91_clean %>% 
  mutate(dist_name = str_remove_all(geo, "[0-9]+"), # creating the "dist_name" variable by removing numbers from the "geo" variable
         dist_name = str_remove_all(dist_name, "\\(\\)"), # removing the empty parentheses
         dist_name = str_remove_all(dist_name, "\\([^()]+\\)"), # there is still information in parentheses (the district names' translation), we want to get rid of that
         dist_nb = str_extract_all(geo, "\\([^()]+\\)")) %>% # creating the "dist_nb" variable by extracting what's in parentheses from the "geo" variable (I could not only extract digits because there are other digits, which have nothing to do with the district numbers)
  unnest(dist_nb) %>% # separating the information we extracted from parentheses
  filter(str_detect(dist_nb, "-", negate = TRUE), 
         str_detect(dist_nb, "[:alpha:]", negate = TRUE)) %>% # remove observations where the district number is not only a number
  mutate(dist_nb = str_extract_all(dist_nb, "(?<=\\().+?(?=\\))")) %>% # again, extract the numbers from their parenteses
  unnest(dist_nb) %>% # I don't think this line and the next are necessary in 1991, but in other years it is 
  filter(str_detect(dist_nb, " ", negate = TRUE)) %>% 
  mutate(dist_name = str_replace_all(dist_name, " - ", "-"), # the next 5 lines are to make sure that district names have the same spelling (or almost) in every census recode
         dist_name = str_replace_all(dist_name, " -", "-"),
         dist_name = str_replace_all(dist_name, "- ", "-"), # this could probably be done in a more tidy way
         dist_name = str_replace_all(dist_name, "--", "-"),
         dist_name = str_trim(dist_name, side = "both"),
         dist_name = ifelse(dist_name == "Timiskaming", "Timiskaming-French River", dist_name)) %>% # Timiskaming is called Timiskaming French River in the 1996 census (first back-and-forth!)
  select(-geo, -ro) # removing unecessary variables -- we want every data frames to have the same variables
...
d91_clean <- d91_clean %>% # we want to have one observation per electoral district, with variables for each age category
  spread(age, value)
d96_clean <- d96_clean %>% 
  spread(age, value)

ro1987 <- rbind(d91_clean, d96_clean) %>% # we then bind the 2 dataframes (vertically, one on top of the other)
  gather(age, value, `0`:`90`) %>% # again, we gather the age variables, now we have one observation for each district-census-age combination (long format)
  spread(census, value) # finally, we come back to a wide format by spreading the values for each age category
...
parl34 <-  read.csv("documentation/data/input/parl_34.csv", sep = ";") %>% 
  mutate(type = Type.of.Parliamentarian, # rename
         ...
         type = str_replace_all(type, "\\)", "\\),"), # add commas between each "position (date)" combination to be able to separate them afterwards
  ...
  mutate(type_date = str_extract_all(type, "\\([^()]+\\)"), # extract dates from "position (date)" combinations
         type_date = str_extract_all(type_date, "(?<=\\().+?(?=\\))"), 
         riding_date = str_extract_all(riding, "\\([^()]+\\)"), # extract dates from "riding (date)" combinations
         riding_date = str_extract_all(riding_date, "(?<=\\().+?(?=\\))")) %>%  
  unnest(c(type_date, riding_date)) %>% 
  mutate(dist_name = str_remove_all(riding, "\\(\\)"), # clean-up riding names: remove what's in parentheses
         dist_name = str_remove_all(dist_name, "\\([^()]+\\)"),
         dist_name = str_replace_all(dist_name, "--", "-")) %>%  
  filter(!str_detect(type, "Senator"), # remove senators
         type_date != 0 | riding_date != 0, # remove rows where dates are not identical: this is an important step, because we've duplicated (our even tripled, quadrupled) the number of observations when separating the position and riding rows in the beginning. Now we want to keep only those where the dates match, because they're the "real" information on MPs: what position they were in period X, and in which riding.
         type_date == riding_date) %>% 
  separate(riding_date, sep = "-", into = c("riding_start_date", "riding_end_date")) %>% # divide start and end dates of sittings
  ...
  mutate(riding_start_date = ymd(riding_start_date), # transform to date format
         ...
         riding_end_date = ifelse(is.na(riding_end_date) == TRUE, "2020-04-01", riding_end_date), # add end date to those who are still serving
         riding_end_date = as.Date(riding_end_date)) %>% 
  filter(riding_end_date >= "1988-11-21" & riding_start_date <= "1993-09-08") %>%  # keep only those who stopped serving after the election (Nov 21, 1988) and those who started before the end of the 34th parliament (Sept 8, 1993) -- because otherwise there are observations for MPs who stayed in the same riding for a longer time than the 34th parliament
...
# We could not easily clean up the party variable, because we have one observation per MP (so their party affiliation matches the caucus they were in during the 34th parliament). I haven't done so but you could use the code we've already used to get rid of what is inside the parentheses.
```
...
census_for_district <- imputation_data2 %>% 
  ...
  filter(year ==  1991 & ro == "ro1987" | 
         ...
         year ==  2015 & ro == "ro2013") %>% # keeping election years for the appropriate representation orders
  ...
  mutate(dist_name = str_replace_all(dist_name, "  ", " "), # removing extra space in-between words
         dist_name = str_replace_all(dist_name, " ", "-")) # making names as similar as possible to candidates data

# We will be merging each parliament dataframe individually to keep track of potential mistakes
# I mentioned earlier that choosing the 1996 district names upon merging censuses could create problems later on. 
...
# After trying to merge once, I found other spelling discrepancies. This is why I am re-spelling some districts here.
# This is a very manual task but I could not find an alternative.
...
election88 <- census_for_district %>% 
  ...
  mutate(dist_name = recode(dist_name, "Laval-Est" = "Laval-East"),
         ...
         dist_name = recode(dist_name, "Mont-Royal" = "Mount-Royal"), # these are basically lost in translation
...
```{r}
join34 <- left_join(parl34, election88) # I also really love the join() functions (full_join, left_join, etc.). I had never used it that much before starting this project
